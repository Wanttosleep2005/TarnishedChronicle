import { damageAfterDefense, effectiveEnemyDefense, estimateEnemyHit } from '../src/renderer/src/lib/build-insights.ts';
...
const enemy = {
  ...
  newGameDefenseScale: { physical: 1.2 },
  newGameStatusScale: { bleed: 1.1 },
...
if (Math.abs(effectiveEnemyDefense(enemy, 'physical', 2) - 135.3) > 0.001) {
  throw new Error('NG+2 defense must apply per-enemy and ClearCount scaling');
}
if (Math.round(ngPlusTwoResult.damage) !== 311 || ngPlusTwoResult.hitsToKill !== 6 || ngPlusTwoResult.poiseHits !== 5) {
  throw new Error(`NG+2 enemy scaling was not applied: ${JSON.stringify(ngPlusTwoResult)}`);
}
