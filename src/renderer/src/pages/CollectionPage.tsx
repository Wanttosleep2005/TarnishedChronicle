...
function entryStatus(entry: CollectionEntry, locationsReady: boolean): CollectionStatus {
  ...
  if (locationsReady && entry.sources.length === 0) return 'unresolved';
  return 'missing';
}
...
function CollectionEntryCard({
  ...
  const acquisitionLabels = [...new Set(entry.sources.map((source) => source.sourceLabel))];
  return (
    <article className={`collection-entry ${status}`}>
      <div className="collection-entry-head">
        ...
        <div className="collection-entry-title">
          ...
          {entry.name !== entry.en && <div className="en-name">{entry.en}</div>}
        </div>
        <span
      ...
      {locationsReady && (
        <div className="collection-acquisition">
          <span className="collection-acquisition-label">获取方式</span>
          <span>{acquisitionLabels.length > 0 ? acquisitionLabels.join(' / ') : '获取方式未收录'}</span>
        </div>
      )}
      {!entry.owned && !locationsReady && <div className="collection-location-loading">正在读取可靠来源…</div>}
      {!entry.owned && locationsReady && primary && (
        <div className="collection-source-primary">
          <div>
            ...
            <div className="collection-source-detail">
              {primary.region ?? '未归类'} · {sourceChance(primary.chance)}
            </div>
          </div>
          <button
      ...
      {!entry.owned && locationsReady && entry.sources.length === 0 && (
          ...
          <div className="collection-source-list">
            {entry.sources.slice(1).map((source) => (
              <button key={`${source.mapId}-${source.source}-${source.projected.px}`} className="collection-source-row" type="button" onClick={() => onLocate(entry, source)}>
                <span>{source.region ?? '未归类'} · {source.sourceLabel}</span>
...
export function CollectionPage() {
  ...
  const visibleEntries = useMemo(() => {
    ...
    return locatedCatalog.entries
      ...
      .filter((entry) => {
        ...
        return `${entry.name} ${entry.en} ${entry.category} ${groupLabel}`.toLowerCase().includes(normalizedQuery);
      })
  ...
  const locate = (entry: CollectionEntry, source: CollectionEntry['sources'][number]) => {
    requestMapFocus({ ...source.projected, name: `${entry.name} · ${source.sourceLabel}` });
  };
