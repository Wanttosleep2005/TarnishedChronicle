...
import { weaponCombatForId } from '../lib/combat-data.ts';
import { zhItemNameByKind } from '../data/zh/translations.ts';
import { inferBuildTags } from '../lib/build-insights.ts';
...
export function BuildPage() {
  ...
  const ranking = profile.weaponVariants
    ...
    .filter((r) => !search.trim() || fuzzyMatch(search, r.name))
    .sort((a, b) => b.ar - a.ar);

  const setAttr = (key: keyof AttrState, value: number) =>
    setAttrs({ ...sim, [key]: Math.min(99, Math.max(1, value)) });
  const buildTags = inferBuildTags(profile, ranking[0]?.id ?? null);
