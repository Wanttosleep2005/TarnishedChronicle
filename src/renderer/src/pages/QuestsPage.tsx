...
import { exportQuestToMarkdown } from '../lib/quest-export.ts';
...
function getMindmapNodes(quests: readonly QuestView[], relations: readonly MindmapRelation[]): readonly MindmapNode[] {
  ...
  return [
    ...place(sorted(false), 5, 135, 210, 185),
    ...place(sorted(true), 3, 1_105, 230, 205),
  ];
}
...
function QuestImpactDesk({
  ...
  const [exportedQuestId, setExportedQuestId] = useState<string | null>(null);
  ...
  const exportRoute = () => {
    const markdown = exportQuestToMarkdown(activeQuest);
    const url = URL.createObjectURL(new Blob([markdown], { type: 'text/markdown;charset=utf-8' }));
    const link = document.createElement('a');
    link.href = url;
    link.download = `${activeQuest.npc}-NPC任务路线.md`.replace(/[<>:"/\\|?*]/g, '-');
    document.body.appendChild(link);
    link.click();
    link.remove();
    URL.revokeObjectURL(url);
    setExportedQuestId(activeQuest.id);
  };
  ...
  return (
    <section className="quest-impact-desk" aria-label="NPC 命运推演台">
      <div className="quest-impact-heading">
        ...
        <button className="btn small" type="button" onClick={exportRoute}>
          {exportedQuestId === activeQuest.id ? '路线档案已导出' : '导出路线档案'}
        </button>
      </div>
  ...
  return (
    <section className="quest-compass" aria-label="NPC 任务路线罗盘">
      <div className="quest-compass-heading">
        <div>
          <span>路线罗盘</span>
          <strong>{activeQuest.npc} 的行动航线</strong>
        </div>
...
function QuestRewardAtlas({
  ...
  const filteredRewardEntries = useMemo(() => {
    ...
    return rewardEntries.flatMap((entry) => {
      ...
      const matches = sources.some(({ quest, stage, reward }) => [entry.name, quest.npc, stage.region, stage.location, reward.branch ?? '']
        .join(' ')
...
function QuestMindmap({
  ...
  const [constellationMode, setConstellationMode] = useState<ConstellationMode>('relations');
  ...
  const nodes = useMemo(() => getMindmapNodes(quests, relations), [quests, relations]);
  ...
  const handlePointerDown = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    if (event.target instanceof Element && event.target.closest('.quest-mindmap-node')) return;
    event.currentTarget.setPointerCapture(event.pointerId);
    drag.current = { pointerId: event.pointerId, startX: event.clientX, startY: event.clientY, x: view.x, y: view.y, moved: false };
  };
  ...
  return (
    <section className="quest-mindmap" aria-label="NPC 任务星图">
      <div className="quest-mindmap-heading">
        ...
        <div className="quest-mindmap-zoom" aria-label="星图缩放">
          <button className="icon-button" type="button" title="缩小" aria-label="缩小任务星图" onClick={() => changeZoom(-0.12)}>−</button>
          <span>{Math.round(view.scale * 100)}%</span>
          <button className="icon-button" type="button" title="放大" aria-label="放大任务星图" onClick={() => changeZoom(0.12)}>+</button>
          <button className="btn small" type="button" onClick={() => setView(MINDMAP_INITIAL_VIEW)}>重置视图</button>
        </div>
      </div>
      <div className="quest-mindmap-toolbar">
        <div className="quest-mindmap-filters" role="group" aria-label="任务星图筛选">
          {(
            [
      ...
      <div className="quest-constellation-focus">
        <div className="quest-constellation-identity">
          <span>当前星体</span>
          <strong>{focusedQuest?.npc ?? '未选择路线'}</strong>
          <small>{focusedQuest ? `${focusedQuest.current.region} · ${focusedQuest.current.location}` : '选择星图节点查看路线脉络'}</small>
          <button
        ...
        <div className="quest-constellation-metrics" aria-label="当前路线概览">
          <span><b>{focusedConnections.length}</b>牵连路线</span>
          <span><b>{focusedRewards.length}</b>可得奖励</span>
          <span className={(focusedQuest?.warnings.length ?? 0) > 0 ? 'has-risk' : ''}><b>{focusedQuest?.warnings.length ?? 0}</b>关键风险</span>
        </div>
        <div className="quest-constellation-mode" role="group" aria-label="星图焦点">
          {(
            [
              ['relations', '关联星系'],
              ['rewards', '战利品星轨'],
              ['warnings', '分歧风险'],
            ] as [ConstellationMode, string][]
          ).map(([key, label]) => (
            <button key={key} className={`btn small ${constellationMode === key ? 'primary' : ''}`} type="button" aria-pressed={constellationMode === key} onClick={() => setConstellationMode(key)}>{label}</button>
          ))}
        </div>
      ...
      >
        <div className={`quest-mindmap-stage mode-${constellationMode} ${activeQuestId ? 'has-focus' : ''}`} style={stageStyle}>
          <svg className="quest-mindmap-lines" viewBox="0 0 1700 960" aria-hidden="true">
            <defs>
          ...
          {visibleNodes.some(({ quest }) => !quest.dlc) && <div className="quest-mindmap-world world-base" aria-hidden="true"><span>交界地</span><small>陆地 · 地下</small></div>}
          {visibleNodes.some(({ quest }) => quest.dlc) && <div className="quest-mindmap-world world-dlc" aria-hidden="true"><span>幽影地</span><small>黄金树幽影</small></div>}
